...
import io # Added for BytesIO for Excel export
...
def add_excel_download_button(df_or_dict_of_dfs, filename_prefix: str, button_label_suffix: str, plot_key: str):
    """
    Adds a Streamlit download button to save DataFrame(s) to an Excel file.

    Args:
        df_or_dict_of_dfs: A single pandas DataFrame or a dictionary of DataFrames
                           where keys are sheet names and values are DataFrames.
        filename_prefix: A prefix for the downloadable Excel file (e.g., "waterbody_index").
        button_label_suffix: Suffix for the button label (e.g., "Plot Data").
        plot_key: A unique key for the Streamlit download button.
    """
            ...
            if isinstance(df_or_dict_of_dfs, pd.DataFrame):
                ...
                for sheet_name, data_df in df_or_dict_of_dfs.items():
                    if isinstance(data_df, pd.DataFrame) and not data_df.empty:
                        sane_sheet_name = re.sub(r'[\[\]\*\/\\?\:\']', '_', str(sheet_name))[:31] # Ensure sheet name is valid
...
def analyze_sampling_generic(sampling_points, first_image_data, first_transform,
    ...
    if os.path.exists(str(lake_height_path)):
        ...
        except Exception: df_h = pd.DataFrame(columns=['Date','Height']) # Ensure df_h is defined
    ...
    if not df_h.empty: fig_dual.add_trace(go.Scatter(x=df_h['Date'],y=df_h['Height'],name='Στάθμη Λίμνης',mode='lines'),secondary_y=False) # Renamed for clarity in lake_height_only
...
def run_lake_processing_app(waterbody: str, index_name: str):
    with st.container():
        st.markdown('<div class="custom-card">', unsafe_allow_html=True) # Corrected class name
...
def analyze_sampling_for_dashboard(sampling_points: list, first_image_data_rgb, first_image_transform,
    ...
    all_mg_vals_date_d={};
    for p_n in pts_plot: # Use pts_plot (selected points) for aggregation
...
def run_water_quality_dashboard(waterbody: str, index_name: str):
    with st.container():
        ...
        common_filename_prefix_dash = f"{waterbody}_{index_name}" # Simpler prefix
        ...
        # Store sampling points lists for Excel export later
        def_pts_list_for_export = []
        upl_pts_list_for_export = []
        
        # Store selected points for filtering exports
        sel_pts_def_names_for_export = []
        sel_pts_upl_names_for_export = []
        ...
        with tabs_ctrl[0]: # Default Sampling
            ...
            st.session_state[f"def_pts_list{key_suffix_dash}"] = def_pts_list # Store for access in button click
            
            if def_pts_list:
                sel_pts_def_names = st.multiselect("Σημεία (Προεπιλογή):", [n for n,_,_ in def_pts_list], default=[n for n,_,_ in def_pts_list], key=f"sel_def{key_suffix_dash}")
                st.session_state[f"sel_pts_def_names{key_suffix_dash}"] = sel_pts_def_names # Store for access
            ...
            if SESSION_KEY_DEFAULT_RESULTS_DASHBOARD in st.session_state and st.session_state[SESSION_KEY_DEFAULT_RESULTS_DASHBOARD]:
                ...
                current_sel_pts_def_names = st.session_state.get(f"sel_pts_def_names{key_suffix_dash}", [p[0] for p in current_def_pts_list])
                ...
                if isinstance(res_def, tuple) and len(res_def) == 7:
                    ...
                    with n_tabs_def_display[0]: # GeoTIFF
                        ...
                        if current_def_pts_list:
                            df_sampling_pts = pd.DataFrame(current_def_pts_list, columns=['PointName', 'Longitude', 'Latitude'])
                            add_excel_download_button(df_sampling_pts, f"{common_filename_prefix_dash}_default", "Sampling Points", f"excel_geo_def_{tab_prefix_key}")
                    ...
                    with n_tabs_def_display[3]: # Χρώματα Pixel & Στάθμη
                        ...
                        if res_c_data: 
                            for point_name, data_list in res_c_data.items():
                                if data_list and (not current_sel_pts_def_names or point_name in current_sel_pts_def_names):
                    ...
                    with n_tabs_def_display[4]: # Μέσο mg/m³
                        ...
                        temp_all_mg_by_d = {}
                        points_for_mean = current_sel_pts_def_names if current_sel_pts_def_names else list(res_m_data.keys())
                        for p_name_temp in points_for_mean:
                            if p_name_temp in res_m_data:
                                for d_obj_temp, val_mg_temp in res_m_data[p_name_temp]:
                                    temp_all_mg_by_d.setdefault(d_obj_temp, []).append(val_mg_temp)
                        
                        s_dts_mg_temp = sorted(temp_all_mg_by_d.keys())
                        mean_mg_temp = [np.mean(temp_all_mg_by_d[d]) for d in s_dts_mg_temp if temp_all_mg_by_d[d]]
                        if s_dts_mg_temp and mean_mg_temp:
                            df_mean_mg = pd.DataFrame({'Date': s_dts_mg_temp, 'Mean_mg_m3': mean_mg_temp}).sort_values(by="Date")
                    ...
                    with n_tabs_def_display[5]: # Συνδυασμένο
                        ...
                        # Re-use s_dts_mg_temp and mean_mg_temp from previous tab if available
                        df_mean_mg_for_dual = pd.DataFrame({'Date': s_dts_mg_temp, 'Mean_mg_m3': mean_mg_temp}) if 's_dts_mg_temp' in locals() and s_dts_mg_temp and 'mean_mg_temp' in locals() and mean_mg_temp else pd.DataFrame(columns=['Date', 'Mean_mg_m3'])
                    ...
                    with n_tabs_def_display[6]: # mg/m³ ανά Σημείο
                        point_options_for_detail = current_sel_pts_def_names if current_sel_pts_def_names else list(res_m_data.keys())
                        if not point_options_for_detail: 
                            ...
                            if sel_pt_d_disp and res_m_data.get(sel_pt_d_disp): 
                                ...
                                # ... (rest of plot and caption logic)
                else: 
        ...
        with tabs_ctrl[1]: # Upload KML
            ...
            if upl_file:
                ...
                if upl_pts_list:
                    ...
                    sel_pts_upl_names = st.multiselect("Σημεία (KML):", [n for n,_,_ in upl_pts_list], default=[n for n,_,_ in upl_pts_list], key=f"sel_upl_{key_suffix_dash}")
            ...
            if SESSION_KEY_UPLOAD_RESULTS_DASHBOARD in st.session_state and st.session_state[SESSION_KEY_UPLOAD_RESULTS_DASHBOARD]:
                ...
                current_sel_pts_upl_names = st.session_state.get(f"sel_pts_upl_names{key_suffix_dash}", [p[0] for p in current_upl_pts_list])
                ...
                if isinstance(res_upl, tuple) and len(res_upl) == 7:
                    ...
                    with n_tabs_upl_display[0]: # GeoTIFF
                        st.plotly_chart(fig_g_u, use_container_width=True, key=f"geo_u_chart_disp_{tab_prefix_key_upl}")
                        if current_upl_pts_list:
                             df_sampling_pts_upl = pd.DataFrame(current_upl_pts_list, columns=['PointName', 'Longitude', 'Latitude'])
                             add_excel_download_button(df_sampling_pts_upl, f"{common_filename_prefix_dash}_upload", "Sampling Points", f"excel_geo_upl_{tab_prefix_key_upl}")
                        if index_name == "Χλωροφύλλη": st.pyplot(create_chl_legend_figure())
                    ...
                    with n_tabs_upl_display[3]: # Χρώματα
                        ...
                        if res_c_data_u:
                            for point_name, data_list in res_c_data_u.items():
                                if data_list and (not current_sel_pts_upl_names or point_name in current_sel_pts_upl_names):
                                    df_point_colors = pd.DataFrame(data_list, columns=['Date', 'RGB_Normalized'])
                                    df_point_colors['R_norm'] = df_point_colors['RGB_Normalized'].apply(lambda x: x[0])
                                    df_point_colors['G_norm'] = df_point_colors['RGB_Normalized'].apply(lambda x: x[1])
                                    df_point_colors['B_norm'] = df_point_colors['RGB_Normalized'].apply(lambda x: x[2])
                        ...
                        if index_name == "Χλωροφύλλη": c2u_disp.pyplot(create_chl_legend_figure("vertical"))

                    with n_tabs_upl_display[4]: # Μέσο mg/m³
                        st.plotly_chart(fig_m_u, use_container_width=True, key=f"mg_u_chart_disp_{tab_prefix_key_upl}")
                        temp_all_mg_by_d_u = {}
                        points_for_mean_u = current_sel_pts_upl_names if current_sel_pts_upl_names else list(res_m_data_u.keys())
                        for p_name_temp in points_for_mean_u:
                            if p_name_temp in res_m_data_u:
                                for d_obj_temp, val_mg_temp in res_m_data_u[p_name_temp]:
                                    temp_all_mg_by_d_u.setdefault(d_obj_temp, []).append(val_mg_temp)
                        
                        s_dts_mg_temp_u = sorted(temp_all_mg_by_d_u.keys())
                        mean_mg_temp_u = [np.mean(temp_all_mg_by_d_u[d]) for d in s_dts_mg_temp_u if temp_all_mg_by_d_u[d]]
                        if s_dts_mg_temp_u and mean_mg_temp_u:
                            df_mean_mg_u = pd.DataFrame({'Date': s_dts_mg_temp_u, 'Mean_mg_m3': mean_mg_temp_u}).sort_values(by="Date")
                            add_excel_download_button(df_mean_mg_u, f"{common_filename_prefix_dash}_upload", "Mean mg_m3", f"excel_mean_mg_upl_{tab_prefix_key_upl}")
                    
                    with n_tabs_upl_display[5]: # Διπλό
                        st.plotly_chart(fig_d_u, use_container_width=True, key=f"dual_u_chart_disp_{tab_prefix_key_upl}")
                        df_mean_mg_for_dual_u = pd.DataFrame({'Date': s_dts_mg_temp_u, 'Mean_mg_m3': mean_mg_temp_u}) if 's_dts_mg_temp_u' in locals() and s_dts_mg_temp_u and 'mean_mg_temp_u' in locals() and mean_mg_temp_u else pd.DataFrame(columns=['Date', 'Mean_mg_m3'])
                    ...
                    with n_tabs_upl_display[6]: # mg/m³ ανά Σημείο
                        point_options_for_detail_u = current_sel_pts_upl_names if current_sel_pts_upl_names else list(res_m_data_u.keys())
...
def run_predictive_tools(waterbody: str, initial_selected_index: str):
    with st.container():
        st.markdown('<div class="custom-card">', unsafe_allow_html=True) # Corrected class
