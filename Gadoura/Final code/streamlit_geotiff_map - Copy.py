...
GADOURA_ROOT = APP_DIR.parent
DATA_ROOT    = GADOURA_ROOT / "DATA"

# Hard-coded fallback (Windows path from user spec)
_FALLBACK = Path(r"C:\Users\ilioumbas\Documents\GitHub\platform\Gadoura_platform\Gadoura\DATA")
if not DATA_ROOT.exists() and _FALLBACK.exists():
    DATA_ROOT    = _FALLBACK
    GADOURA_ROOT = _FALLBACK.parent
...
def load_chl_points(csv: str) -> pd.DataFrame:
    p = Path(csv)
    if not p.exists():
        return pd.DataFrame(columns=["date","point","chl_a","color"])
    ...
    df = pd.DataFrame({
        "date":  pd.to_datetime(details.str.extract(r"(\d{4}-\d{2}-\d{2})", expand=False), errors="coerce"),
        "point": pd.to_numeric(raw.iloc[:, 1], errors="coerce"),
        "chl_a": pd.to_numeric(details.str.extract(r"Chl-a[^:]*:\s*(-?\d+(?:\.\d+)?)", expand=False), errors="coerce"),
        "color": styles.str.extract(r"fill-color:\s*(#[0-9A-Fa-f]{6})", expand=False).str.upper(),
    }).dropna(subset=["date","chl_a"])
...
def main() -> None:
    ...
    cols = st.columns(len(CASE_CONFIG))
    for col, cfg in zip(cols, CASE_CONFIG):
