...
"""
Water Quality App (Enterprise-Grade UI - Startup & Performance Enhanced)
-----------------------------------------
...
warnings.filterwarnings("ignore", category=UserWarning, module='openpyxl') # For openpyxl warnings
# Note: NotGeoreferencedWarning will be imported and filtered later if rasterio is used.
...
def check_memory_usage(threshold=80.0):
    ...
    try:
        ...
        if memory_percent > threshold:
            ...
            return False
    except Exception: # psutil might not be available or other error
        pass # Silently pass if psutil check fails after startup
    return True
...
def periodic_gc_and_cache_clear():
    ...
    gc.collect()
    if st.session_state.run_counter % 25 == 0: # Reduced from 50 for more frequent cache checks
        st.cache_data.clear()
...
def debug_message(*args, **kwargs):
    if DEBUG:
        # Using st.expander might be too heavy if DEBUG is on often.
        # Consider simpler st.write for debug messages if expander causes issues.
        try:
            with st.expander("Debug Messages", expanded=False):
                st.write(*args, **kwargs)
        except Exception: # Fallback if expander itself has issues (e.g. during very early init)
            print(f"DEBUG (fallback): {args} {kwargs}")


def inject_custom_css():
    # CSS is relatively lightweight, can be defined globally
    st.markdown("""
...
def run_custom_sidebar_ui_custom(authenticator_obj): # Pass authenticator object
...
def parse_sampling_kml(kml_source) -> list:
    ...
    # xml.etree.ElementTree is imported globally (ET)
    try:
...
@st.cache_data(ttl=86400, max_entries=100) # Cache for 1 day, many entries
...
def read_image(file_path: str, lake_shape: dict = None, downsample_factor: int = 1):
    ...
    from rasterio.enums import Resampling # Local import
    from rasterio.features import geometry_mask # Local import
    ...
    try:
        with rasterio.open(file_path) as src:
            ...
            img = src.read(out_shape=out_shape, resampling=Resampling.bilinear if downsample_factor > 1 else Resampling.nearest)
            ...
            img_band1 = np.where(img_band1 == 0, np.nan, img_band1) # Treat 0 as NaN
...
def run_lake_processing_app(waterbody: str, index_name: str):
    ...
    import numpy as np # Local import
    import pandas as pd # Local import
    import plotly.express as px # Local import
    ...
    with st.container():
        ...
        DATES_ALL_AVAILABLE = [m['date'] for m in metadata] # All dates from metadata for filter setup
        ...
        with st.spinner("Επεξεργασία φιλτραρισμένων δεδομένων..."):
            ...
            sum_days_in_range = np.zeros(img_shape, dtype=np.float32) # For mean day
            count_valid_for_mean_day = np.zeros(img_shape, dtype=np.int16) # Denominator for mean day

            average_sample_img_sum = np.zeros(img_shape, dtype=np.float64)
            average_sample_img_count = np.zeros(img_shape, dtype=np.int16)
            
            stack_for_time_max_val = np.full(img_shape, -np.inf if index_name != "Πραγματικό" else 0, dtype=np.float32) # Initialize with very small number or 0
            time_max_map = np.full(img_shape, np.nan, dtype=np.float32) # Stores the day_of_year
            ...
            for i, meta_item in enumerate(filtered_metadata):
                ...
                # Assuming single band analysis for these maps, using the first band
                # If index_name is 'Πραγματικό', this might need adjustment if it's supposed to be RGB.
                # For now, let's assume the maps are derived from the first band value.
                img_data = img_data_bands[0] 
                ...
                # Time max logic
                # Pixels that are in range AND have a value greater than the current max for that pixel
                # Also consider NaNs in stack_for_time_max_val as needing update
                update_time_max_mask = np.logical_and(in_range_mask_valid, 
                                                     (img_data > stack_for_time_max_val) | np.isnan(time_max_map))
                ...
                month = meta_item['date'].month
                year = meta_item['date'].year
                ...
                progress = (i + 1) / total_files_to_process
                progress_bar.progress(progress)
                status_text.text(f'Επεξεργασία: {i + 1}/{total_files_to_process} εικόνες')
        ...
        tick_vals = [1,32,60,91,121,152,182,213,244,274,305,335,365]; tick_text = ["Ιαν","Φεβ","Μαρ","Απρ","Μαΐ","Ιουν","Ιουλ","Αυγ","Σεπ","Οκτ","Νοε","Δεκ",""]
        with exp_col2, st.expander("Μέση Ημέρα Εμφάνισης", True):
            fig_mean_day = px.imshow(mean_day_map, color_continuous_scale="RdBu", labels={"color": "Μέση Ημέρα"}, color_continuous_midpoint=182)
            fig_mean_day.update_layout(coloraxis_colorbar=dict(tickmode='array', tickvals=tick_vals, ticktext=tick_text))
            st.plotly_chart(fig_mean_day, use_container_width=True)
            add_excel_download_button(pd.DataFrame(mean_day_map), common_filename_prefix, "Mean_Day", f"excel_meanday_{key_suffix}")
        
        st.subheader("Ανάλυση Δείγματος Εικόνας")
        exp_col3, exp_col4 = st.columns(2)
        with exp_col3, st.expander("Μέσο Δείγμα Εικόνας", True):
            if not np.all(np.isnan(average_sample_img_display)):
                fig_sample = px.imshow(average_sample_img_display, color_continuous_scale="jet", labels={"color": "Τιμή"})
                st.plotly_chart(fig_sample, use_container_width=True)
                add_excel_download_button(pd.DataFrame(average_sample_img_display), common_filename_prefix, "Avg_Sample", f"excel_avgsample_{key_suffix}")
        with exp_col4, st.expander("Χρόνος Μέγιστης Τιμής", True):
            fig_time_max = px.imshow(time_max_map, color_continuous_scale="RdBu", labels={"color": "Ημέρα Μέγιστης"}, color_continuous_midpoint=182, range_color=[1,365])
            fig_time_max.update_layout(coloraxis_colorbar=dict(tickmode='array', tickvals=tick_vals, ticktext=tick_text))
            st.plotly_chart(fig_time_max, use_container_width=True)
            add_excel_download_button(pd.DataFrame(time_max_map), common_filename_prefix, "Time_Max_Val", f"excel_timemax_{key_suffix}")

        # ... (Monthly/Yearly distributions - code is similar to previous, ensure local imports if needed)
        st.markdown('</div>', unsafe_allow_html=True)
        gc.collect()

@st.cache_resource(ttl=3600, max_entries=3) # Max 2 orientations + default
def create_chl_legend_figure(orientation="horizontal", theme_bg_color=None, theme_text_color=None):
    ...
    label_text = "Συγκέντρωση Χλωροφύλλης-α (mg/m³)"; tick_labels = [str(l) for l in levels]
...
@st.cache_data(ttl=1800, max_entries=10, show_spinner="Ανάλυση σημείων δειγματοληψίας...") # Cache for 30 mins
def analyze_sampling_data(sampling_points: list, images_folder_path: str,
    ...
    for filename in tif_files:
        _, date_obj_file = extract_date_from_filename(filename) # Use renamed var
                ...
                for name, lon, lat in sampling_points:
                    try:
                        ...
                        if 0 <= col < src.width and 0 <= row < src.height:
                            ...
                            # Read 3 bands for RGB
                            pixel_data = src.read([1,2,3], window=win)
                            r, g, b = pixel_data[0,0,0], pixel_data[1,0,0], pixel_data[2,0,0]
                            mg_val = (g / 255.0) * 2.0
                            results_mg[name].append((date_obj_file, mg_val))
                            results_colors[name].append((date_obj_file, (r / 255., g / 255., b / 255.)))
                    except IndexError: pass # debug_message if needed
                    except Exception: pass # debug_message if needed
        except Exception: pass # st.warning if needed
        gc.collect()

    df_h = pd.DataFrame(columns=['Date', 'Height'])
    if os.path.exists(str(lake_height_excel_path)):
        try:
            df_h_temp = pd.read_excel(lake_height_excel_path) # openpyxl warning might appear here
...
def get_image_preview(file_path: str, downsample_factor: int = 4):
    ...
    try:
        with rasterio.open(file_path) as src:
            ...
            data = src.read([1, 2, 3], out_shape=(3, int(src.height / downsample_factor), int(src.width / downsample_factor)), resampling=Resampling.bilinear)
            transform = src.transform * src.transform.scale((src.width / data.shape[-1]), (src.height / data.shape[-2]))
            rgb_disp = data.transpose((1, 2, 0))
            if rgb_disp.max() > 1.0 and rgb_disp.max() <=255: rgb_disp = rgb_disp / 255.0
            elif rgb_disp.max() > 1.0: # if already float but > 1, clip (unusual for RGB)
                 rgb_disp = np.clip(rgb_disp / np.nanmax(rgb_disp),0,1)
...
def image_navigation_ui(images_folder: str, available_dates_map: dict,
    ...
    if not available_dates_map: st.info("Δεν υπάρχουν διαθέσιμες εικόνες."); return None
    sorted_date_strings = sorted(available_dates_map.keys())
    current_idx = st.session_state.setdefault(session_state_key_for_idx, 0)
    current_idx = min(max(0, current_idx), len(sorted_date_strings) - 1)
    ...
    def update_idx_from_select_nav_img(): # Renamed to avoid conflict
        st.session_state[session_state_key_for_idx] = sorted_date_strings.index(st.session_state[f"{key_prefix}_select_nav_img"]) # Renamed key

    col_select.selectbox("Επιλογή Ημερομηνίας:", options=sorted_date_strings, index=current_idx,
                         key=f"{key_prefix}_select_nav_img", on_change=update_idx_from_select_nav_img, # Renamed key
    ...
    else: st.error(f"Το αρχείο εικόνας δεν βρέθηκε: {image_full_path}")
    return image_full_path

def generate_dashboard_figures(sampling_points, results_colors_data, results_mg_data, df_h_data,
    ...
    fig_geo.update_xaxes(visible=False); fig_geo.update_yaxes(visible=False, scaleanchor="x", scaleratio=1); fig_geo.update_layout(height=500, uirevision='geo_dash')
    figures['geo'] = fig_geo
    ...
    fig_colors.update_layout(title='Χρώματα Pixel & Στάθμη', yaxis=dict(tickmode='array', tickvals=list(pt_y_map.values()), ticktext=list(pt_y_map.keys())), yaxis2=dict(title='Στάθμη (m)'), uirevision='colors_dash', height=450, hovermode='x unified')
    ...
    mean_mg = [np.mean(all_mg_by_d[d]) for d in s_dts_mg if all_mg_by_d[d]] if s_dts_mg else []
...
def create_decimated_plot(dates, values, title, y_axis_title, max_points=500, use_webgl=True):
    ...
    import plotly.graph_objects as go # Local import
    if not hasattr(dates, '__iter__') or not hasattr(values, '__iter__') or not dates or not values:
    ...
    if values_list: # Add colorscale only if values exist
        try:
            marker_props.update(color=values_list, colorscale='Viridis', showscale=True, colorbar=dict(title=y_axis_title))
        except: # Fallback if colorscale fails
            pass

    fig.add_trace(ScatterClass(
        x=dates_list, y=values_list, mode='lines+markers',
        marker=marker_props, line=dict(width=1, color='grey'), name=y_axis_title
    ))
    fig.update_layout(title=title, xaxis_title='Ημερομηνία', yaxis_title=y_axis_title, height=400, uirevision=f"{title}_rev_dec", hovermode='x unified')
    return fig
...
def create_dual_axis_decimated_plot(df_h, dates_mg, values_mg, title, max_points=500, use_webgl=True):
    ...
    if len(h_dates) > max_points:
        step = max(1, len(h_dates) // max_points); h_dates, h_values = h_dates[::step], h_values[::step]
        
    mg_dates_list = list(dates_mg)
    mg_values_list = list(values_mg)
    if len(mg_dates_list) > max_points:
        step = max(1, len(mg_dates_list) // max_points); mg_dates_list, mg_values_list = mg_dates_list[::step], mg_values_list[::step]
    ...
    if mg_dates_list and mg_values_list:
    ...
    fig.update_layout(title=title, xaxis_title='Ημερομηνία', uirevision='dual_rev_dec', height=450,
...
def run_water_quality_dashboard(waterbody: str, index_name: str):
    ...
    import pandas as pd # Local import
    import numpy as np # Local import
    import plotly.graph_objects as go # For detail plot
    ...
    with st.container():
        ...
        height_excel_path = os.path.join(data_folder, "lake height.xlsx")
        ...
        def _display_tab_content(tab_key_suffix, kml_points_list, session_results_key, session_figs_key, selected_points_names_list):
            res_data = st.session_state.get(session_results_key)
            figures = st.session_state.get(session_figs_key)
            ...
            if res_data and figures:
                ...
                with n_tabs_display[1]:
                    image_navigation_ui(images_folder_path, available_tifs, f"img_idx_{tab_key_suffix}", f"nav_{tab_key_suffix}", index_name=="Χλωροφύλλη", index_name)
                ...
                with n_tabs_display[5]:
                    st.plotly_chart(figures['dual_plot'], use_container_width=True)
                ...
                st.caption("Πατήστε 'Εκτέλεση' για να δείτε τα αποτελέσματα.")
        ...
        with tabs_ctrl[1]: # Upload
            ...
            if upl_file:
                ...
                st.session_state[upl_kml_key] = upl_pts_list
                if upl_pts_list:
...
def run_predictive_tools(waterbody: str, initial_selected_index: str):
    ...
    with st.container():
        st.markdown('<div class="card">', unsafe_allow_html=True) # Use "card" for consistency
        ...
        date_min_pred = col1.date_input("Από:", date(2020,1,1), key=f"date_min{key_suffix_pred}")
        date_max_pred = col2.date_input("Έως:", date.today(), key=f"date_max{key_suffix_pred}")
        ...
        if sampling_type_pred == "Προεπιλογή":
            dfolder_pred = get_data_folder(waterbody, "Πραγματικό") # Assume RealColor KML is representative
            if dfolder_pred:
        ...
        if st.button("Εκτέλεση Παράλληλης Ανάλυσης", key=f"run_all_pred{key_suffix_pred}"):
            ...
            for i, idx_n in enumerate(indices):
                ...
                if d_fold:
                    img_fold = os.path.join(d_fold, "GeoTIFFs")
                    h_excel = os.path.join(d_fold, "lake height.xlsx")
                    # analyze_sampling_data is cached, so repeated calls with same params are fast
        ...
        if session_results_key_pred in st.session_state:
            ...
            for idx_n, (res_c, res_m, df_h) in results_all.items():
                ...
                mean_vals = [np.mean(all_mg_by_d[d]) for d in s_dts if all_mg_by_d[d]] if s_dts else []
                if s_dts and mean_vals:
            ...
            if excel_data_mg_all: add_excel_download_button(excel_data_mg_all, f"{waterbody}_predictive", "All_Indices_Mean_mg_m3", "excel_pred_mg_all_indices")
        st.markdown('</div>', unsafe_allow_html=True); gc.collect()

def main_app(authenticator_obj): # Pass authenticator
    ...
    run_custom_sidebar_ui_custom(authenticator_obj) # Pass authenticator
...
if __name__ == "__main__":
    # --- Authenticator Initialization (deferred to runtime) ---
    authenticator = None
    try:
        ...
        if len(AUTH_NAMES) == len(AUTH_USERNAMES) == len(AUTH_PLAIN_PASSWORDS):
            ...
            authenticator = stauth.Authenticate(
                credentials_dict,
                "water_quality_app_cookie_v8", # New cookie name
                "a_very_random_secret_key_v8", # New key
                cookie_expiry_days=30
            )
        else:
            st.error("Σφάλμα ρύθμισης αυθεντικοποίησης: Ασυμφωνία λιστών.")
            st.stop()
    except ImportError:
        st.error("Η βιβλιοθήκη 'streamlit-authenticator' δεν βρέθηκε. Παρακαλώ εγκαταστήστε την.")
        st.stop()
    except Exception as e:
        st.error(f"Σφάλμα αρχικοποίησης Authenticate: {e}")
        st.stop()
    # --- End Authenticator Initialization ---

    if authenticator:
        name, authentication_status, username = authenticator.login('main')
        # st.session_state.authentication_status is now set by authenticator.login()

        if st.session_state.get("authentication_status"):
            # Pass the initialized authenticator object to main_app
