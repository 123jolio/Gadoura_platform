...
def _depth_profiles_chart_board_png(
    ...
    for row_idx, date_str in enumerate(dates_sorted):
        ...
        for col_idx, param in enumerate(profile_params):
            ...
            ax.set_ylabel("Βάθος (m)" if col_idx == 0 else "", fontsize=9, color="#0f172a", fontfamily="DejaVu Sans")
...
def _build_chla_summary(df_in):
    ...
    for d in sorted(src["date"].dropna().unique()):
        ...
        # Drop obvious Secchi outliers (e.g. ~30 m artifact) from summary stats.
            ...
            for row_idx, sel_date_dp in enumerate(dates_sorted):
                ...
                for i, param in enumerate(profile_params):
                    ...
                    fig_p.update_layout(
                        ...
                        yaxis=dict(title="Βάθος (m)", range=depth_axis_range),
                        height=380,
                        plot_bgcolor="#ffffff",
