...
def run_lake_processing_app(waterbody: str, index_name: str):
    with st.container():
        ...
        if not indices_to_keep:
            st.info("Δεν υπάρχουν δεδομένα για την επιλεγμένη περίοδο/μήνες/έτη. Παρακαλώ προσαрμόστε τα φίλτρα.")
            st.markdown('</div>', unsafe_allow_html=True); return
                    ...
                    with n_tabs_def_display[2]:
                        if vid_path:
                            if vid_path.endswith(".mp4"):
                                # --- MODIFICATION START ---
                                st.video(vid_path, key=f"video_def{key_suffix_dash}") # ADDED KEY
                                # --- MODIFICATION END ---
                            else:
                                # --- MODIFICATION START ---
                                st.image(vid_path, key=f"image_def{key_suffix_dash}") # ADDED KEY
                                # --- MODIFICATION END ---
                ...
                if isinstance(res_u,tuple) and len(res_u)==7:
                    ...
                    with n_tabs_upl_display[2]:
                        if vid_path:
                            if vid_path.endswith(".mp4"):
                                # --- MODIFICATION START ---
                                st.video(vid_path, key=f"video_upl{key_suffix_dash}") # ADDED KEY
                                # --- MODIFICATION END ---
                            else:
                                # --- MODIFICATION START ---
                                st.image(vid_path, key=f"image_upl{key_suffix_dash}") # ADDED KEY
                                # --- MODIFICATION END ---
                            if index_name=="Χλωροφύλλη":
                                st.pyplot(create_chl_legend_figure())
                        else:
...
def run_predictive_tools(waterbody: str, initial_selected_index: str):
    with st.container():
        ...
        with col_filt2:
            ...
            date_max_common = st.date_input("Ημερομηнία έως:", value=date.today(), key=f"date_max_common{key_suffix_pred_section}")
        ...
        if st.button("Εκτέλεση Παράλληλης Ανάλυσης & Εμφάνιση Αποτελεσμάτων", key=f"recalc_parallel{key_suffix_pred_section}", type="primary", use_container_width=True):
            ...
            with st.spinner("Εκτέλεση αναλύσεων για όλους τους δείκτες... Αυτό μπορεί να διαρκέσει λίγο."):
                ...
                # Αποθήκευση των αποτελεσμάτων στο session state
        ...
        if f"predictive_tool_results{key_suffix_pred_section}" in st.session_state:
            ...
            indices_to_analyze = ["Πραγματικό", "Χλωροфύλλη", "Θολότητα"]
            ...
            for chart_name_key, fig_internal_key in chart_display_options.items():
                ...
                if chart_name_key == "Χρώματα Pixel & Στάθμη":
                    for idx_name_iter in indices_to_analyze:
                        ...
                        st.markdown("---" if idx_name_iter != indices_to_analyze[-1] else "")
                else:
                        ...
                        with inner_cols[i]:
                            ...
                            if fig_internal_key == "lake_height_only":
                                ...
                                if fig_dual_source and hasattr(fig_dual_source, 'data') and fig_dual_source.data:
                                    height_trace = next((trace for trace in fig_dual_source.data if hasattr(trace, 'name') and (trace.name == 'Στάθμη' or trace.name == 'Ύψος Λίμνης')), None) # Adjusted name check
                                    if height_trace:
                                        df_h_data = result_for_idx.get("df_h")
                                        title_text = f"Στάθμη Λίμνης ({idx_name_iter})"
                                        if df_h_data is None or df_h_data.empty:
                                             title_text += " (Δεν βρέθηκαν δεδομένα Excel)"
                                    ...
                                    st.caption(f"Δεν βρέθηκε πηγή (dual fig) για στάθμη ({idx_name_iter}).") # More specific caption
                            else:
                                fig_to_plot = result_for_idx.get(fig_internal_key)
                            ...
                            if fig_to_plot:
                                ...
                                if fig_internal_key == "geo" and idx_name_iter == "Χλωροφύλλη":
                                    st.pyplot(create_chl_legend_figure(orientation="horizontal"))
                            elif fig_internal_key != "lake_height_only":
